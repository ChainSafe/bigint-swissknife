...
You can find typedoc documentation [here](https://chainsafe.github.io/bigint-swissknife/modules/_chainsafe_bigint-uint8array.html).
...
The library uses the NAPI bindings when available (when bigint-buffer is installed as peer dependency). Besides the default `converter` there is also the [`uncheckedConverter`](https://chainsafe.github.io/bigint-swissknife/variables/_chainsafe_bigint-uint8array.uncheckedConverter-1.html)
